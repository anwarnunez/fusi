...
def sparse_matrix_from_cluster_times(spike_times,
                                     spike_clusters,
                                     nclusters=None,
                                     spike_counts=None):
    ...
    spike_counts : optional, 1D np.ndarray (n,)
        Spike counts for each cluster.
        Defaults to None, assuming one spike per time point.
    ...
    shape = (nspikes, nclusters)

    # determine maximum abount of spikes per bin
    dtype = np.bool if spike_counts is None else lowest_uint(nspikes)

    if spike_counts is None:
        # one time point is one spike
        spike_counts = np.ones_like(spike_times)

    assert len(spike_counts) == len(spike_times)

    sparse_matrix = sparse.csr_matrix((spike_counts,
                                       (np.arange(nspikes), spike_clusters)),
                                      shape, dtype=dtype)
...
def event_related_responses(spike_times,
                            spike_clusters,
                            event_times,
                            duration=1.0,
                            dt=0.01,
...
def event_related_responses_from_sparse(spike_times,
                                        sparse_matrix,
                                        event_times,
                                        duration=1.0,
                                        dt=0.01,
    ...
    spike_times    : 1D float np.ndarray (n,)
        Time of spikes in [seconds]
    sparse_matrix : 1D int scipy.linalg.sparse (n, nclusters)
        Vector containing the cluster ID for every spike
    event_times    : 1D float np.ndarray (m,)
        Onset of event in [seconds]
    duration : scalar or 1D np.ndarray (m,)
...
class ProbeSpikes(object):
    ...
    def psth(self, event_times, dt=0.05, duration=1.0):
        ...
        '''
        event_spike_times, event_spike_matrix = event_related_responses_from_sparse(self.times,
                                                                                    self.sparse_matrix,
                                                                                    event_times,
                                                                                    duration=duration,
                                                                                    dt=dt,
